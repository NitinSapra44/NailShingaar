...
const NAIL_SHAPES: {
  id: NailQuestionnaire['nail_shape'];
  label: string;
  desc: string;
  image: string;
}[] = [
  { id: 'Square',  label: 'Square',  desc: 'Straight sides with a flat tip.',       image: '/nail-shapes/nail-square.jpg' },
  { id: 'Squoval', label: 'Squoval', desc: 'Square shape with softened corners.',    image: '/nail-shapes/nail-squoval.jpg' },
  { id: 'Round',   label: 'Round',   desc: 'Rounded edges, natural curve.',          image: '/nail-shapes/nail-round.jpg' },
  { id: 'Oval',    label: 'Oval',    desc: 'Tapered sides with a rounded tip.',      image: '/nail-shapes/nail-oval.jpg' },
  { id: 'Almond',  label: 'Almond',  desc: 'Tapered sides with a rounded point.',   image: '/nail-shapes/nail-almond.jpg' },
  { id: 'Coffin',  label: 'Coffin',  desc: 'Tapered sides with a flat tip.',         image: '/nail-shapes/nail-coffin.jpg' },
];
...
const Checkout = () => {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      for (const slot of REQUIRED_SLOTS) {
        ...
        if (file) {
          photoUrls.push(await uploadFile('nail-photos', file, `${user.id}/${ts}_${slot.key}.${file.name.split('.').pop()}`));
        }
      }
      ...
      const screenshotUrl = await uploadFile(
        'payment-screenshots',
        paymentScreenshot,
        `${user.id}/${ts}_payment.${paymentScreenshot.name.split('.').pop()}`
      );
  ...
  return (
    <>
      <Helmet><title>Order — {product.name} | Nail Shingaar by Reet</title></Helmet>
      <Layout>
                ...
                <div className="space-y-8 animate-fade-up">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {NAIL_SHAPES.map((shape) => {
                        const selected = questionnaire.nail_shape === shape.id;
                        return (
                          <button
                            key={shape.id}
                            type="button"
                            onClick={() => setQuestionnaire({ ...questionnaire, nail_shape: shape.id })}
                            className={`flex flex-col items-center gap-2 px-2 pt-4 pb-3 rounded-2xl border-2 transition-all ${
                              selected
                          ...
                          >
                            ...
                            <div className="text-center">
                              <p className={`text-xs font-bold leading-tight tracking-wide uppercase ${selected ? 'text-primary' : 'text-foreground'}`}>
                                {shape.label}
                              </p>
                              <p className="text-[9px] text-muted-foreground leading-snug mt-0.5">{shape.desc}</p>
                            </div>
                          </button>
                        );
                      })}
                  ...
                  <div className="space-y-2">
                    <Label className="text-base font-semibold">
                      3. Colour Preference <span className="font-normal text-muted-foreground">(optional)</span>
                    </Label>
                    <Textarea
                      placeholder="e.g. Nude with gold glitter, deep burgundy, pastel pink French tips…"
                    ...
                    <div className="flex items-start gap-2 px-4 py-3 bg-ink-light rounded-xl border border-border text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-muted-foreground text-xs mt-0.5">
                          Place a ₹5 or ₹10 coin flat beside your nails in each photo. Lay your hand flat, ensure good lighting, and capture all fingers clearly.
                        </p>
                      </div>
                    </div>
                ...
                <div className="space-y-6 animate-fade-up">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      After paying, take a screenshot and upload it here. We'll verify and confirm your order within a few hours.
                    </p>
                    {paymentPreview ? (
